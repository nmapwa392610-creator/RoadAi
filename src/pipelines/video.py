...
def process_video(video_path, frame_skip=20):
    ...
    seen_boxes = []         # уже виденные bbox (для null случаев)
                ...
                for det in detections:
                    ...
                    if tid is not None:
                        if tid not in seen_track_ids:
                            # проверяем по IoU — вдруг это null который уже видели
                            ...
                            if not is_dup:
                                ...
                                # дубликат null — просто запоминаем track_id
                                seen_track_ids.add(tid)

                    else:
                        # track_id = null — проверяем по IoU
                        is_dup = any(calc_iou(box, seen) >= 0.4 for seen in seen_boxes)
                        if not is_dup:
                            seen_boxes.append(box)
                            new_detections.append(det)
